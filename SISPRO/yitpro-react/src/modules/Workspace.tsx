...
const WorkSpace = () => {
  ...
  const [editingTabId] = useState(null);
  useEffect(() => {
    // Inyectar estilos
  ...
  const CrearWorkSpace = async (values) => {
    try {
      ...
      if (res.data.Exito) {
        ...
        const workspace = JSON.parse(res.data.WorkSpace);
  ...
  const ConsultaWorkSpace = async (Id) => {
    try {
      ...
      if (res.data.Exito) {
        ...
        setAvailableTabs(JSON.parse(res.data.Views))
  ...
  const handleTabSelect = async (selectedTab) => {
    try {
      ...
      if (res.data.Exito) {
        ...
        const workspacetab = JSON.parse(res.data.WorkSpaceTab);
